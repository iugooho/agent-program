...
由 `scaffold-cli` 生成的 Java 后端骨架（Maven + JDK {{javaVersion}}）。
...
├── scripts/                                 # 常用命令封装
└── src
    ├── main/java/{{packagePath}}/App.java    # 入口类
    ├── main/resources/logback.xml           # 日志配置
    └── test/java/{{packagePath}}/AppTest.java
```
...
- 编译目标 JDK {{javaVersion}}，编译参数带 `-parameters -Xlint:all`
